...
export default function BasicTable() {
  ...
  useEffect(() => {
    getAllRegistersUsersStatus().then((data) => setPerson(data));
  }, []);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    await deletePersonStatusCertificateId({ idMedical: idModal }).catch(
      (error) => {
        // Handle the error
        console.error(error);
      }
  ...
  const handleSubmitEdit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    await updateStatusCertificateByAdmin({
      status: status,
      idMedical: idModal,
    }).catch((error) => {
      // Handle the error
            ...
            <TableBody>
              {persons?.map((person) => (
                <TableRow key={person.idmedicalCertificate}>
                  ...
                  <TableCell>
                    <Button onClick={() => handleDownload(person.attachment)}>
                      <CloudDownloadIcon />
                    </Button>
                  </TableCell>
                  ...
                  <TableCell>
                    <CircleIcon
                      color={`${setStatusColorIcon(person.status)}`}
                    />
                  </TableCell>
                  <TableCell>
                    <Button
        ...
        <Dialog open={isOpen} onClose={() => setIsOpen(false)}>
          <form onSubmit={handleSubmit}>
            ...
            >
              ...
              <DialogContent>{nameModal}</DialogContent>
              <Box
                display={"flex"}
                flexDirection={"column"}
                gap={"10px"}
